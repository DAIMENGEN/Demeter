  ...
  const handleSubmit = async () => {
    try {
      ...
      if (type === "user") {
        ...
        if (defaultUserId) {
          const allowed = (values.optionsRows ?? []).some((r) => {
            const raw = r.value;
            const id = typeof raw === "string" ? raw : raw?.value;
            return id === defaultUserId;
          });
      ...
      if (type === "select" || type === "user") {
        ...
        for (const row of values.valueColorMapRows ?? []) {
          ...
          if (key && !allowedKeys.has(key)) {
            message.error("值颜色映射中的值必须来自已配置的可选范围，请先补充可选值/人员或删除该映射");
            return;
          }
        }
      }
      ...
      const options =
        type === "select" || type === "user" ? rowsToOptionsJson(values.optionsRows) : null;
                  ...
                  type === "select" || type === "user"
                    ? (optionsRows
                        .map((r) => {
                          ...
                          // select: raw is string
                          // user: raw is { value: userId, label?: ReactNode }
                          const key = typeof raw === "string" ? raw : raw?.value;
                          if (!key) return null;

                          const labelStr = (r?.label ?? "") as unknown as string;
                          return {
                            value: key,
                            label: labelStr || key
                          };
